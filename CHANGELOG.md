...
+ [优化] 内置提示词来源支持读取部署目录挂载的本地镜像，并兼容 Markdown 与 HTML 图片。
+ [新增] 支持通过一次性交接票据安全导入 New API 的 Image 分组令牌并自动配置画布渠道。
+ [调整] New API 令牌交接渠道显示为「千模岛 AI」，并移除系统内置默认渠道。
+ [调整] 顶部状态栏移除文档、版本和 GitHub 外部入口。
